...
public interface WorkRepository extends JpaRepository<WorkDto, Integer> {
	@Query(value = "SELECT wok.work_id, wok.pay_date, wok.user_id, wok.work_item_id, wok.child_comment, "
			+ "wok.attached_image, wok.parent_comment_id, wok.approval , item.name as work_item_name, item.currency_id,item.number,null as approval_message "
			+ "FROM spajam.t_work wok INNER JOIN spajam.m_work_item item ON item.work_item_id = wok.work_item_id where wok.user_id=:userId", nativeQuery = true)
